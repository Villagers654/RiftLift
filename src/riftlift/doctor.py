...
def doctor(paths: Paths) -> int:
    ...
    check("Platform bridge", lambda: paths.tools / "platform-compat/LibOVRPlatformImpl64_1.dll" if (paths.tools / "platform-compat/LibOVRPlatformImpl64_1.dll").is_file() else (_ for _ in ()).throw(FileNotFoundError("not installed")))

...
bool ProfileManager::Initialize()
{
	...
	if (!m_ProfileWindow)
	{
		m_ProfileWindow = glfwCreateWindow(PROFILE_WINDOW_WIDTH, PROFILE_WINDOW_HEIGHT, "RiftLift Microprofile", NULL, NULL);
		if (!m_ProfileWindow)
			return false;
	...
	{
		vr::VROverlay()->CreateOverlay("riftlift.debug.microprofile", "RiftLift Microprofile", &m_ProfileOverlay);
